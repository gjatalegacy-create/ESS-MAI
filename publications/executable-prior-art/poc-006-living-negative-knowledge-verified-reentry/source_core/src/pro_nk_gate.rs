...
impl KnowledgeVault {
    ...
    pub fn from_negative_export(bytes: &[u8]) -> Self {
            ...
            NkMagic::Nkl1 => {
                ...
                match (
                    ...
                    (_, false) => {
                        super::runtime_pulse::mark_notready(super::runtime_pulse::Stage::NkImport);
                        eprintln!(
                            "  [NK_BRIDGE] ALARM: CRC NKL1 s'përputhet — vault bosh i deklaruar"
                        );
                        return Self::new();
                    }
                }
        ...
        // Header minimal 4 byte. Një histori e vulosur me trup të cunguar nuk
        // mund të degradojë në vault bosh: statusi bëhet NOT_READY dhe main-i
        // ndalon PRO-n kur historia sovrane ekziston.
        let has_header = body.len() >= 4;
        let count = match has_header {
            true => u32::from_le_bytes([body[0], body[1], body[2], body[3]]) as usize,
            false => 0,
        };
        let count_bounded = count <= body.len().saturating_sub(4) / 4;
        let entries_to_parse = match count_bounded {
            true => count,
            false => 0,
        };
        let mut cursor = 4usize;
        let mut parse_valid = has_header & count_bounded;
        for _ in 0..entries_to_parse {
            let has_len = cursor.saturating_add(4) <= body.len();
            ...
            let entry_end = cursor.saturating_add(entry_len);
            let has_entry = entry_len > 0 && entry_end <= body.len();
            match has_entry {
                true => {
                    parse_valid &= vault.absorb_shadow_entry(&body[cursor..entry_end]);
                    cursor = entry_end;
                }
                false => parse_valid = false,
            }
        }
        parse_valid &= cursor == body.len();
        match parse_valid {
            true => vault,
            false => {
                super::runtime_pulse::mark_notready(super::runtime_pulse::Stage::NkImport);
                eprintln!("  [NK_BRIDGE] ALARM: struktura e export-it NK është e cunguar ose ka byte të tepërt — vault-i refuzohet");
                Self::new()
            }
        }
    }
    ...
    fn absorb_shadow_entry(&mut self, slice: &[u8]) -> bool {
        // Minimumi për header 36 byte. Legacy entries pa raw evidence mbeten të
        // lexueshme; një raw_len i pranishëm por i cunguar refuzohet.
        match slice.len() >= 36 {
            true => {
                ...
                let proc_len =
                    u32::from_le_bytes([slice[32], slice[33], slice[34], slice[35]]) as usize;
                let proc_end = 36usize.saturating_add(proc_len);
                let process = match slice.get(36..proc_end) {
                    Some(bytes) => String::from_utf8_lossy(bytes).to_string(),
                    None => return false,
                };
                let raw_state = match slice.get(proc_end..proc_end.saturating_add(4)) {
                    Some(raw_len_bytes) => {
                        let mut raw_len_word = [0u8; 4];
                        raw_len_word.copy_from_slice(raw_len_bytes);
                        let raw_len = u32::from_le_bytes(raw_len_word) as usize;
                        let raw_start = proc_end.saturating_add(4);
                        let raw_end = raw_start.saturating_add(raw_len);
                        match slice.get(raw_start..raw_end) {
                            Some(bytes) => (bytes, raw_end == slice.len()),
                            None => (&[][..], false),
                        }
                    }
                    None => (&[][..], proc_end == slice.len()),
                };
                match raw_state.1 {
                    true => {}
                    false => return false,
                ...
                let living_material =
                    match shadow_contracts::negative_asset::NegativeAssetEnvelope::decode(
                        raw_state.0,
                    ) {
                        Ok(asset) => asset.semantic_feedback_text(&process),
                        Err(_) => process,
                    };
                let id = format!("shadow_nk_{}", input_id);
                self.negative
                    .push(NegativeEntry::from_text(&id, &living_material));
                true
            }
            false => false,
        }
...
impl NkGate {
    ...
    pub fn pre_constraint(vault: &mut KnowledgeVault, vectors: &[FragmentVector]) -> NkVerdict {
...
mod tests {
    ...
    fn nk_seal_roundtrip_preserves_body() {
        ...
        assert_eq!(
            &sealed[0..4],
            b"NKL1",
            "koka magjike v1.4.3 = NKL1 (me gjatësi)"
        );
        assert_eq!(nk_blob_magic(&sealed), NkMagic::Nkl1);
        let vault = KnowledgeVault::from_negative_export(&sealed);
    ...
    fn nk_corrupt_raises_presume_notready() {
        ...
        assert!(
            crate::runtime_pulse::nk_status().is_ready(),
    ...
    fn nk_legacy_after_nkb1_is_downgrade() {
        ...
        assert!(
            nk_downgrade(true, false),
            "vulë e parë + raw tani = DEGRADIM"
        );
        assert!(!nk_downgrade(true, true), "vulë + vulë = normale");
        assert!(
            !nk_downgrade(false, false),
            "kurrë vulë + raw = legacy i pastër"
        );
    ...
    fn nk_seal_detects_single_bit_flip() {
        ...
        assert_eq!(
            v.len(),
            0,
            "NK i prishur duhet të japë vault bosh, jo dije të korruptuar"
        );
    }
    ...
    fn post_filter_blocks_known_failure() {
        ...
        vault.add_negative(NegativeEntry::from_text(
            "fail1",
            "perpetual motion energy free infinite",
        ));
