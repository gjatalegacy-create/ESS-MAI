...
/// Faza e kolapsit ka një identitet fizik të vetëm në gcl-constitution.
/// Ky re-export ruan path-in ekzistues `lab_contracts::collapse::CollapsePhase`.
pub use gcl_constitution::CollapsePhase;
...
impl PrimitiveSplit {
    ...
    pub fn from_wire(xi_seg: &str, yi_seg: &str) -> Option<Self> {
        ...
        Some(Self {
            xi: split_terms(xi_body),
            yi: split_terms(yi_body),
        })
    }
}
...
mod tests {
    ...
    fn split_empty_yi_allowed_but_both_empty_no_material() {
        let only_xi = PrimitiveSplit {
            xi: vec!["kimi".into()],
            yi: vec![],
        };
        assert!(only_xi.has_material());
        let none = PrimitiveSplit {
            xi: vec![],
            yi: vec![],
        };
    ...
    fn outcome_labels_binary_semantics() {
        let p = CollapseOutcome::Proposed {
            x: "flamuri".into(),
            y: "shqipëria".into(),
            mass: 0.83,
        };
        let r = CollapseOutcome::Refused {
            reason: "XiYi=∅".into(),
        };
