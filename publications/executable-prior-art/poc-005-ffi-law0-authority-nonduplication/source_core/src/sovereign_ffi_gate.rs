...
use crate::types::{LawConfirmedVerdict, SupremeVerdict};
...
pub fn seal_verified_output(
    ...
    Ok(verification_receipt_from_token(
        ...
        pd_binding_digest, pd_continuum_activation_digest, verdict.as_supreme(),
    ))
}
...
#[cfg(any(test, feature = "legacy_ffi_compat"))]
#[no_mangle]
...
///        -7 = VaultWriteFailed (legacy; nuk prodhohet nga surface-i i mbyllur)
///        -8 = CompatibilityHold (mungon committed verdict; zero write, zero copy)
///
/// # Safety
...
/// LAW_3: ABI legacy mbetet i ekspozueshëm vetëm me feature të shprehur,
/// por nuk mund të materializojë dije pa një verdict të komituar. Payload-i
/// lexohet si borrowed view mbi të njëjtin pointer dhe nuk kopjohet.
#[cfg(any(test, feature = "legacy_ffi_compat"))]
unsafe fn sovereign_commit(
    _token: LgcToken,
    handle: CapHandle,
    data_ptr: *const u8,
    data_len: usize,
) -> i32 {
    let data = match unsafe { borrow_ffi_payload(data_ptr, data_len) } {
        Ok(data) => data,
        Err(code) => return code,
    };
    let input_id = fnv(data);
    let proof = [handle.gen, handle.nonce];
    let lineage = crate::knowledge_lineage::Lineage::derive_capability(input_id, &proof);
    match lineage.is_traceable() {
        false => -4,
        true => {
            eprintln!(
                "[SOVEREIGN] CompatibilityHold: direct FFI write kërkon committed verdict"
            );
            -8
        }
    }
...
/// # Safety
/// Caller-i garanton pointer të lexueshëm për `data_len` dhe lifetime deri në
/// kthimin e thirrjes. View-i ruan adresën bazë; Rust nuk e liron memorien.
#[cfg(any(test, feature = "legacy_ffi_compat"))]
unsafe fn borrow_ffi_payload<'a>(
    data_ptr: *const u8,
    data_len: usize,
) -> Result<&'a [u8], i32> {
    match data_ptr.is_null() || data_len == 0 {
        true => Err(-5),
        false => {
            // SAFETY: kontrata e caller-it garanton pointer, gjatësi, alignment
            // byte-wise, immutable aliasing dhe lifetime për gjithë thirrjen.
            Ok(unsafe { std::slice::from_raw_parts(data_ptr, data_len) })
        }
    }
}
...
mod tests {
    ...
    fn verification_output_is_sealed_by_single_use_token() {
        ...
        let confirmed = crate::types::LawConfirmedVerdict::confirm(verdict, true)
            .expect("verdict kushtetues në test");
        let receipt = seal_verified_output(
            "sess-v", "i0-v", 0xAA, 0xBB, 0xCC, 0xDD, &confirmed,
    ...
    #[test]
    fn nonempty_direct_write_is_compatibility_hold() {
        let data = b"borrowed-ffi-payload";
        let handle = sovereign_issue_capability(S_SHADOW_WRITE);
        // SAFETY: `data` është i lexueshëm për gjithë thirrjen.
        let result = unsafe {
            sovereign_validate_and_write(handle, data.as_ptr(), data.len())
        };
        assert_eq!(result, -8);
    }

    #[test]
    fn ffi_view_preserves_pointer_identity_without_allocation() {
        let data = b"zero-copy-ffi-view";
        // SAFETY: `data` është i lexueshëm për gjithë view-n.
        let view = unsafe { borrow_ffi_payload(data.as_ptr(), data.len()) }
            .expect("borrowed FFI view");
