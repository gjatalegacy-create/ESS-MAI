param(
    [string]$LogRoot = (Join-Path ([Environment]::GetFolderPath('Desktop')) 'ESS_MAI_V156_LOGS')
)
...
try {
    ...
    $Manifest = Join-Path $Root 'ESS_MAI_V1_5_6_FILELIST.sha256'
    $Failures = @()
    ...
    Write-Host "ESS-MAI v1.5.6 validation completed. Logs: $LogRoot"
}
finally {
    Pop-Location
}
