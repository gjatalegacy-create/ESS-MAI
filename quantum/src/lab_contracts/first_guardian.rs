...
mod first_guardian_tests {
    ...
    // ── v1.5.1 (Korrigjimi D, VETËM #[cfg(test)]) ────────────────────────
    // FRYMA_E_FUNDIT është gjendje globale e procesit; nën paralelizëm,
    // attest() i një testi tjetër e mbishkruante mes attest→breath_status
    // të këtij testi (provë: kalonte i izoluar dhe me --test-threads=1).
    // ROJA E RITIT: i njëjti guard për ÇDO test që prek ritin —
    //  • lock i përbashkët (OnceLock<Mutex<()>>, vetëm std);
    //  • state i përcaktuar NË HYRJE (fryma zerohet para ritit);
    //  • RAII: edhe në panic, Drop e rikthen state-in (0 = s'ka rit);
    //  • helmimi i Mutex-it shpërfillet (into_inner) — një test që
    //    paniku s'i rrëzon zinxhir të tjerët; state-i rivendoset gjithsesi.
    // Runtime state machine, DegradedHard, presume dhe Shadow — TË PAPREKURA.
    struct RojeRituali {
        _kyci: std::sync::MutexGuard<'static, ()>,
    }
    impl RojeRituali {
        fn merr() -> Self {
            static ROJA: std::sync::OnceLock<std::sync::Mutex<()>> =
                std::sync::OnceLock::new();
            let kyci = ROJA.get_or_init(|| std::sync::Mutex::new(()))
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
            FRYMA_E_FUNDIT.store(0, std::sync::atomic::Ordering::SeqCst);
            Self { _kyci: kyci }
        }
    }
    impl Drop for RojeRituali {
        fn drop(&mut self) {
            FRYMA_E_FUNDIT.store(0, std::sync::atomic::Ordering::SeqCst);
        }
    }
    ...
    fn full_breath_when_all_organs_beat() {
        let _roje = RojeRituali::merr(); // rit i mbrojtur (state global)
        let checks = [
