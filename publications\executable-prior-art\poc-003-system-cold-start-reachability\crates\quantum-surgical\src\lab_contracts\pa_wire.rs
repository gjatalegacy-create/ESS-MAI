...
pub fn encode_line_verified(
    ...
    match verify_line(trimmed) {
        ...
        PaWireVerdict::Corrupt(_) => Err(
            "invariant i telit: fushat u zhvendosën ('|' në terma?) — rreshti refuzohet në burim",
        ),
    }
}
...
pub fn seal_body_verified(body: &str, legacy_fields: &[usize]) -> Result<String, &'static str> {
    match body.contains('\n') {
        true => return Err("invariant i telit: '\\n' brenda trupit — rreshti do të copëtohej"),
        false => {}
    }
...
mod pa_wire_tests {
    ...
    fn legacy_three_and_five_fields_declared_ok() {
        assert_eq!(
            verify_line("S1|00000000000000ff|9"),
            PaWireVerdict::LegacyOk
        );
        assert_eq!(
            verify_line("S1|00000000000000ff|9|xi:a|yi:b"),
            PaWireVerdict::LegacyOk
        );
    }
    ...
    fn encode_verified_clean_terms_are_sealed() {
        ...
        let line =
            encode_line_verified("SES_V", "00000000000000aa", 7, "xi:flamur,kuq|yi:shqiponje")
                .expect("terma të pastër duhet të vulosen");
        assert_eq!(
            verify_line(line.trim_end_matches('\n')),
    ...
    fn generic_seal_pd_roundtrip_7_to_8_is_sealed() {
        ...
        assert_eq!(
            verify_line_generic(line.trim_end_matches('\n'), &[6, 7], 8),
            PaWireVerdict::SealedOk
        );
    }
    ...
    fn generic_legacy_counts_declared_and_alien_corrupt() {
        ...
        assert_eq!(
            verify_line_generic("a|b|c|d|e|f", &[6, 7], 8),
            PaWireVerdict::LegacyOk
        );
        assert_eq!(
            verify_line_generic("a|b|c|d|e|f|g", &[6, 7], 8),
            PaWireVerdict::LegacyOk
        );
