...
impl ConstitutionalInvariant {
    pub fn text(&self) -> &'static str {
        match self {
            Self::LightAnchored => "Asgjë nuk hyn pa u bërë Primitive_Anchor.",
            Self::QuantumCollapsed => "Asgjë nuk del pa kaluar Quantum_Collapse.",
            Self::ShadowVerified => "Asgjë nuk pranohet pa Sovereign_Verification_Collapse.",
        }
    }
...
impl LawViolation {
    pub fn report(&self) -> String {
        format!(
            "LAW0 SHKELJE [{}::{}] pasiguria u rrit: {:.2} → {:.2}",
            self.phase.label(),
            self.module,
            self.before,
            self.after
...
pub fn proclaim_law(platform: &str) {
    println!(
        "╔══ [LIGJI I LEXUAR] platforma={} vula={:016x} ══╗",
        platform,
        law_seal()
    );
    ...
    println!(
        "╠══ REGJISTRI: {} LIGJE AKTIVE · vula {:016x} ══╣",
        SYSTEM_LAWS.len(),
        system_laws_seal()
    );
    println!(
        "╠══ {} ROLE EKZEKUTIVE (JO AGJENTË) ══╣",
        PLATFORM_ROLES.len()
    );
...
mod tests {
    ...
    fn law0_monotone_chain_collapses_to_zero() {
        ...
        l.record(CollapsePhase::Coordination, "primitive_anchor", 16.0, 3.0)
            .unwrap();
        // NPRO eliminon 1 term Xi → 2×1=2.
        l.record(CollapsePhase::Reasoning, "npro", 3.0, 2.0)
            .unwrap();
        // PRO i≈XY: 1 çift mbijeton.
        l.record(CollapsePhase::Reasoning, "pro", 2.0, 1.0).unwrap();
        // MPRO: XY i vetëm i matur.
        l.record(CollapsePhase::Reasoning, "mpro", 1.0, 1.0)
            .unwrap();
        // Shadow: D u vendos → 0.
        l.record(CollapsePhase::Verification, "kolaps3", 1.0, 0.0)
            .unwrap();
    ...
    fn constitutional_invariants_bound_to_phases() {
        assert_eq!(
            ConstitutionalInvariant::LightAnchored.phase(),
            CollapsePhase::Coordination
        );
        assert_eq!(
            ConstitutionalInvariant::QuantumCollapsed.phase(),
            CollapsePhase::Reasoning
        );
        assert_eq!(
            ConstitutionalInvariant::ShadowVerified.phase(),
            CollapsePhase::Verification
        );
        assert!(ConstitutionalInvariant::LightAnchored
            .text()
            .contains("Primitive_Anchor"));
    }
    ...
    fn supreme_directive_issue_verify_roundtrip() {
        ...
        let fake = SupremeDirective {
            phase: CollapsePhase::Reasoning,
            seal: 0,
        };
    ...
    fn platforms_are_roles_not_agents() {
        ...
        assert!(
            plats.contains(&"LIGHT") && plats.contains(&"QUANTUM") && plats.contains(&"SHADOW")
        );
    ...
    fn system_laws_registry_complete_and_sealed() {
        ...
        for c in [
            "LAW-0", "LIGJI-0", "L-FC", "L-500", "L-PA", "L-HW10", "L-MT", "L-CX3", "L-NC", "L-FP",
        ] {
